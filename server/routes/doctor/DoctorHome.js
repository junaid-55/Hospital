...
router.put("/makeprescription/:appointmentId", async (req, res) => {
  try {
    console.log(
      "request coming from /doctorhome/makeprescription/:appointmentId"
    );
  ...
  try {
    ...
    const user = await pool.query(
      `INSERT INTO prescription (prescription_id, disease_name, patient_type, date, advice)
       VALUES ((SELECT prescription_id FROM appointment WHERE appointment_id = $1), $2, $3, $4, $5)
       RETURNING *`,
  ...
  try {
    ...
    const insertedPrescriptions = await Promise.all(
      Test.map(async (test) => {
        ...
        FROM selected_test
        WHEN check_test_exists((SELECT prescription_id FROM appointment WHERE appointment_id = $2), selected_test.test_id) = 0
        RETURNING *;
        `,
  ...
  try {
    ...
    const insertedPrescriptions = await Promise.all(
      drugs.map(async (drug) => {
        ...
        FROM selected_drug
        WHEN Check check_drug_exists((SELECT prescription_id from appointment WHERE appointment_id = $4), drug_id) = 0
        RETURNING *;
        `,
...
router.delete(
  ...
  async (req, res) => {
    try {
      const { id } = req.params;
      const user = await pool.query(
        "DELETE FROM prescription_drug WHERE drug_id = $1",
        [id]
      );
      return res.json(user.rows);
    } catch (err) {
...
router.delete(
  ...
  async (req, res) => {
    try {
      const { id } = req.params;
      const user = await pool.query(
        "Delete from prescription_lab where test_name=$1 and(select from prescription ) ",
        [id]
      );
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      console.log(
        "request coming from /appointments/doctor-appointments/:doctor_id",
        doctor_id
      );
...
router.get("/doctor-surgery", async (req, res) => {
  try {
    console.log(
      "request coming from /appointments/doctor-surgery/:appointmentId"
    );
    const type = req.headers.type;
    console.log(appointmentId);
...
router.get("/doctor-approvedsurgeries/:appointmentId", async (req, res) => {
  try {
    console.log(
      "request coming from /appointments/doctor-approvedsurgeries/:appointmentId"
    );
    const { appointmentId } = req.params;
    const type = req.headers.type;
    console.log(appointmentId);
...
router.get("/doctor-approvesurgery/:appointmentId", async (req, res) => {
  try {
    console.log(
      "request coming from /appointments/doctor-approvesurg/:appointmentId"
    );
    const { appointmentId } = req.params;
    const type = req.headers.type;
    console.log(appointmentId);
